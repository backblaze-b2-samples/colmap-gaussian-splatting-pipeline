...
def get_s3_client():
    return boto3.client(
        ...
        config=Config(
            ...
            user_agent_extra="b2ai-colmap-gaussian-splatting-pipeline",
        ),
    )
...
def invalidate_listing() -> None:
    """Public wrapper so sibling repo modules (repo/artifacts.py) can drop the
    shared full-bucket listing cache after they write or delete objects, without
    reaching into list_cache directly. Keeps /files, the Captures library, and
    the dashboard from serving a just-written capture's outputs stale.
    """
